locals {
  ...
  kafka_log_segment_bytes = var.kafka_log_retention_bytes / 10
}

resource "yandex_mdb_kafka_cluster" "gaming-data-cluster" {
  name        = "${var.project_prefix_name}-kafka"
  environment = strcontains(lower(var.env), lower("prod")) ? "PRODUCTION" : "PRESTABLE"
  network_id  = var.vpc_id
  subnet_ids  = [for s in var.subnets : s.id if strcontains(lower(s.name), lower("kafka"))]

  config {
    version          = var.kafka_version
    brokers_count    = var.kafka_brokers_count
    zones            = [for s in var.subnets : s.zone]
...
resource "yandex_mdb_kafka_topic" events {
  ...
  name               = var.kafka_topics[count.index]["name"]
  partitions         = var.kafka_topics[count.index]["partitions"]
  replication_factor = var.kafka_topics[count.index]["replication_factor"]
}
...
resource "yandex_mdb_kafka_user" "kafka_user" {
  ...
  password   = var.kafka_users[count.index]["name"]
